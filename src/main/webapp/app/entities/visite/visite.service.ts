...
export class VisiteService {
    ...
    private convertDateFromClient(visite: IVisite): IVisite {
        const copy: IVisite = Object.assign({}, visite, {
            date: visite.date != null && visite.date.isValid() ? visite.date.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((visite: IVisite) => {
            visite.date = visite.date != null ? moment(visite.date) : null;
        });
        return res;
    }
}
